...
kubectl --kubeconfig ./config config set-credentials $USERNAME \
  --client-certificate=$(pwd)/$CRT_FILE \
  --client-key=$(pwd)/$KEY_FILE

kubectl --kubeconfig ./config  config set-context $USERNAME-context --cluster=$CLUSTERNAME --namespace=$NAMESPACE --user=$USERNAME
kubectl --kubeconfig ./config  config use-context $USERNAME-context
